...
import { KeywordRank } from 'src/modules/ranking/infrastructure/persistence/keyword-rank.schema';
import {
  computeRankWindow,
  toDateString,
...
import {
  KeywordRankReadModelPort,
  RankedKeywords,
} from 'src/modules/ranking/application/port/keyword-rank-read-model.port';
 ...
 */
@Injectable()
export class KeywordRankService implements KeywordRankReadModelPort {
  ...
  async refresh(): Promise<void> {
    ...
    try {
      const window = computeRankWindow(this.config.keywordWindowDays);
      const ranked = await this.keywordRankingSource.getRanked(
