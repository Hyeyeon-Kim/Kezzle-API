...
describe('KeywordRankService', () => {
  ...
  it('reads only the latest batch without running the aggregation', async () => {
    ...
    const { service, rankModel, keywordRankingSource, findQuery } = createMocks(
      {
    ...
    expect(keywordRankingSource.getRanked).not.toHaveBeenCalled();
  ...
  it('builds the read model synchronously on cold start', async () => {
    ...
    const { service, rankModel, keywordRankingSource } = createMocks({
    ...
    expect(keywordRankingSource.getRanked).toHaveBeenCalledTimes(1);
    const [start, end, topN] = keywordRankingSource.getRanked.mock.calls[0];
  ...
  it('applies the injected window days', async () => {
    const { service, keywordRankingSource } = createMocks({
    ...
    const [start, end] = keywordRankingSource.getRanked.mock.calls[0];
    expect(new Date(end).getTime() - new Date(start).getTime()).toBe(
      7 * DAY_MS,
    );
  });
  ...
  it('keeps serving the previous batch when the refresh fails', async () => {
    ...
    const { service, keywordRankingSource, rankModel } = createMocks({
      latestResults: [stale],
      docs: [{ searchWord: 'birthday', count: 10 }],
    });
    keywordRankingSource.getRanked.mockRejectedValue(
