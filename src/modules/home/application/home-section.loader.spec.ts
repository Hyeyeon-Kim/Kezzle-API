...
describe('HomeSectionLoader', () => {
  ...
  function createLoader(overrides?: {
    ...
    const curationQuery = {
      findFeatured: overrides?.curations ?? jest.fn().mockResolvedValue([]),
    };
    const homeCache = {
  ...
  it('counts only an actual CLIP call failure as an AI error', async () => {
    ...
    await expect(
      databaseFailure.loader.loadAnniversary(250, new AbortController().signal),
    ...
    await expect(
      clipFailure.loader.loadAnniversary(250, new AbortController().signal),
