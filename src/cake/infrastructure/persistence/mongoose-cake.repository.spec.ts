import { MongooseCakeRepository } from './mongoose-cake.repository';
import { CakeNotFoundException } from '../../domain/exception/cake-not-found.exception';

describe('MongooseCakeRepository', () => {
  describe('findByIdOrThrow', () => {
    it('returns the document when found', async () => {
      const doc = { _id: 'cake-1', owner_store_id: 'store-1' };
      const cakeModel = { findById: jest.fn().mockResolvedValue(doc) };
      const repo = new MongooseCakeRepository(cakeModel as any);
