...
import {
  AiSearchCakePageResult,
  AiSearchCakeResult,
} from '../../application/ai-search-result';
import { ClipSearchPort } from '../../application/clip-search.port';
import {
  AiSearchResultMapper,
  ExternalAiSearchCakePageResult,
...
@Injectable()
export class ClipHttpAdapter implements ClipSearchPort {
  constructor(
  ...
  async koSearchPage(
    ...
    return this.measure(ENDPOINT_KO_SEARCH_PAGE, async () => {
      ...
      const request = signal
        ? this.httpService.get<ExternalAiSearchCakePageResult>(apiUrl, {
            signal,
          })
        : this.httpService.get<ExternalAiSearchCakePageResult>(apiUrl);
      const response = await request.toPromise();
      return {
        result: response.data.result.map((cake) =>
          AiSearchResultMapper.toApplication(cake),
        ),
